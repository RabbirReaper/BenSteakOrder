<template>
  <div>
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h2>套餐列表</h2>
      <router-link :to="{ name: 'admin-combo-template-edit', params: { id: 'new' } }" class="btn btn-primary">
        新增套餐
      </router-link>
    </div>
        ...
        <div class="card h-100 hover-shadow" style="cursor: pointer;" @click="goToEdit(template._id)">
          ...
          <div class="card-body">
            ...
            <p class="card-text text-muted">
              {{ template.dishes?.length || 0 }} 個餐點項目
            </p>
            <p class="card-text">
...
const fetchTemplates = async () => {
  ...
  try {
    const response = await api.comboTemplate.getAll();
    
    if (response.data.success) {
      templates.value = response.data.templates || [];
    } else {
      error.value = response.data.message || '獲取套餐列表失敗';
    }
  } catch (err) {
    console.error('Error fetching combo templates:', err);
...
// 跳轉到編輯頁面
const goToEdit = (id) => {
  router.push({ name: 'admin-combo-template-edit', params: { id } });
};
